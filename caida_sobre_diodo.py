...
df = pd.DataFrame(columns=['Tiempo (s)', 'Voltaje total (num)', 
                           'Voltaje caida sobre diodo (R = 1k serie)num)'])
t_medicion = 1
pa = pyaudio.PyAudio()
...
signal_desf = senoidal(f_sampleo=fs, frecuencia=freq, num_puntos=len(datos),
               vpp=vol, offset=0.)
ax[0].plot(signal_desf, label='Voltaje total')
ax[1].plot(datos, label='Caida sobre R')
tiempo = np.linspace(0, t_medicion, len(datos))
df['Tiempo (s)'] = tiempo
df['Voltaje total (num)'] = signal_desf
df['Voltaje caida sobre diodo (R = 1k serie))'] = datos
df.to_csv('caida_diodo_1canal_sobrediodo.dat')
